...
def compute_loss(output, pred_box, gt_box, gt_mask, num_boxes, num_classes, grid_size, image_size):
    ...
    # compute coordinate losses (x, y, w, h)
    for b in range(num_boxes):
        # loss for x and y coordinates
        loss_x += weight_coord * torch.sum(box_mask[:, b] * torch.pow(gt_box[:, 0] - output[:, 5*b], 2.0))
        loss_y += weight_coord * torch.sum(box_mask[:, b] * torch.pow(gt_box[:, 1] - output[:, 5*b+1], 2.0))
        
        # loss for width and height
        loss_w += weight_coord * torch.sum(box_mask[:, b] * torch.pow(torch.sqrt(gt_box[:, 2]) - torch.sqrt(output[:, 5*b+2]), 2.0))
        loss_h += weight_coord * torch.sum(box_mask[:, b] * torch.pow(torch.sqrt(gt_box[:, 3]) - torch.sqrt(output[:, 5*b+3]), 2.0))

    # compute non-object confidence loss
    noobj_mask = 1.0 - box_mask
    for b in range(num_boxes):
        loss_noobj += weight_noobj * torch.sum(noobj_mask[:, b] * torch.pow(0.0 - output[:, 5*b+4], 2.0))

    # compute class probability loss
    if num_classes > 0:
        class_mask = gt_mask.unsqueeze(1).repeat(1, num_classes, 1, 1)
        gt_class = torch.ones(batch_size, num_classes, num_grids, num_grids)  # Assuming single class (cracker box)
        loss_cls = torch.sum(class_mask * torch.pow(gt_class - output[:, 5*num_boxes:], 2.0))
